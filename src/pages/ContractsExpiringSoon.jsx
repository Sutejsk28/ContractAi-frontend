...
function ContractsExpiringSoon({ status }) {
  ...
  useEffect(() => {
    try {
      if (!localStorage.getItem("token")) {
        ...
        const fetch = async () => {
          const response = await axios.get(`${server}/contract/expiring-soon`, {
            headers: {
  ...
  return (
    <>
      <h1 className="text-2xl font-semibold m-3 p-3">
        Contracts Expiring soon
      </h1>
      {contracts.length === 0 ? (
        <p>No Contracts that will be expiring soon</p>
      ) : (
...
export default ContractsExpiringSoon;
